...
def hand_capture(frame_in, box_x, box_y):
    ...
    for i in xrange(capture_box_count):
        ROI[i * capture_box_dim:i * capture_box_dim + capture_box_dim, 0:capture_box_dim] = hsv[box_y[i]:box_y[i] + capture_box_dim,
...
def mark_hand_center(frame_in, cont):
    ...
    print(x, y, w, h)
    for ind_y in xrange(int(y + 0.25 * h),
                        int(y + 0.6 * h)):  # around 0.25 to 0.6 region of height (Faster calculation with ok results)
        for ind_x in xrange(int(x + 0.3 * w),
                            int(x + 0.6 * w)):  # around 0.3 to 0.6 region of width (Faster calculation with ok results)
    #for ind_y in xrange(int(y),
    #                    int(y + h)):  # around 0.25 to 0.6 region of height (Faster calculation with ok results)
    #    for ind_x in xrange(int(x),
    #                        int(x + w)):  # around 0.3 to 0.6 region of width (Faster calculation with ok results)
...
def remove_bg(frame):
    ...
    cv2.imwrite('fg_mask_erode.png', fg_mask)
    frame = cv2.bitwise_and(frame, frame, mask=fg_mask)
    cv2.imwrite('fg_mask_bitwise_and.png', fg_mask)
    return frame
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if (not (capture_done and bg_captured)):
            ...
            cv2.imwrite('hand_threshold.png', frame)
            contour_frame = np.copy(frame)
            contour_frame, contours, hierarchy = cv2.findContours(contour_frame, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
            cv2.imwrite('contour.png', contour_frame)
        ...
        # Capture background by pressing 'b'
            ...
            bg_model = cv2.createBackgroundSubtractorMOG2(500, 16, False)
            bg_captured = 1
